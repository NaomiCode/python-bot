...
BOT_TOKEN = "7301980518:AAEgevV3IWa5PGHDnsg_uZg6onuRsXfdKug"
PAYMENT_CHANNEL = "@kirtukosenamuset"
#add payment channel here including the '@' sign
OWNER_ID = 120112735
#write owner's user id here.. get it from @MissRose_Bot by /id
CHANNELS = ["@kirtukosenamuset"]
...
def check(user_id):
    for i in CHANNELS:
        check_join = bot.get_chat_member(i, user_id)
        if check_join.status != 'left':
            pass
        else:
            return False
    return True
...
def menu(user_id):
    ...
    bot.send_message(user_id, "*🏡 Home*", parse_mode="Markdown",
            ...
            if ch == True:
                ...
                if user not in data['refer']:
                    ...
                    if int(data['referby'][user]) != user_id:
                        ...
                        data['referred'][ref] += 1
                        bot.send_message(
                            ref_id, f"*🏧 New Referral on Level 1, You Got : +{Per_Refer} {TOKEN}*",
                            parse_mode="Markdown")
                        json.dump(data, open('users.json', 'w'))
...
def amo_with(message):
    try:
        ...
        bot.send_message(user_id,
                         "✅* Withdraw is request to our owner automatically\n\n💹 Payment Channel :- " + PAYMENT_CHANNEL + "*",
                         parse_mode="Markdown")

        markupp = telebot.types.InlineKeyboardMarkup()
        markupp.add(telebot.types.InlineKeyboardButton(text='🍀 BOT LINK',
